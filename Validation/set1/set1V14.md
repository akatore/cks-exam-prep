Here is the validation script for the **Pod Security Admission (PSA)** task.

Because PSA acts as an Admission Controller, the ultimate proof that you solved the question correctly is whether the API Server actually allowed the Pod to be created and run. 

This script checks the live state of the Deployment in the cluster to see if it successfully spawned Pods, and then inspects the live Pod template to ensure all the specific `restricted` requirements were met.
